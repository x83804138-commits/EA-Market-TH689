...
export default function PostEAForm({ packages, onBack, onSubmit }: PostEAFormProps) {
  ...
  const [formData, setFormData] = React.useState({
    ...
    description: '',
    category: 'GOLD',
    demoImageFile: null as File | null,
    usersThisMonth: '0',
    usersLastMonth: '0',
    currentUsers: '0',
    rating: '5',
  ...
  const [demoImagePreview, setDemoImagePreview] = React.useState<string | null>(null);
  ...
  React.useEffect(() => {
    return () => {
      if (demoImagePreview) URL.revokeObjectURL(demoImagePreview);
      imagePreviews.forEach(url => URL.revokeObjectURL(url));
    };
  }, [demoImagePreview, imagePreviews]);

  const handleDemoImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    setFormData({ ...formData, demoImageFile: file || null });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.demoImageFile) {
      alert('กรุณาอัปโหลดรูปภาพสาธิตหน้าจอ');
      return;
    }
    ...
    try {
      ...
      const demoImageBase64 = await fileToBase64(formData.demoImageFile);
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-xs font-black uppercase tracking-widest text-gray-500 mb-3">รูปภาพสาธิตหน้าจอ (Demo Image / Screenshot)</label>
                    <div className="border-2 border-dashed border-white/10 rounded-2xl p-8 text-center hover:border-yellow-500/50 transition-colors cursor-pointer group relative overflow-hidden">
                      ...
                      <div className="relative z-0">
                        <p className="text-sm font-bold text-gray-400">
                          {formData.demoImageFile ? formData.demoImageFile.name : 'ลากไฟล์รูปภาพสาธิต หรือคลิกเพื่ออัปโหลด'}
                        </p>
